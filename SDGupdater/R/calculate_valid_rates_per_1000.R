...
calculate_valid_rates_per_1000 <- function (numerator, denominator, decimal_places) {
  ...
  if (any(numerator < 0)) { stop("numerator must be greater than 0") }
  if (any(denominator < 0)) { stop("denominator must be greater than 0") }
  if (decimal_places < 0) { stop("decimal_places must be greater than or equal to 0") }

  if (any(numerator > denominator)) { stop("numerator must be smaller than or equal to denominator") }
